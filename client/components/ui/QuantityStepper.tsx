...
export type QuantityStepperVariant = 'primary' | 'primary-alt' | 'secondary' | 'tertiary';
...
export interface QuantityStepperProps {
  ...
  /** When true, starts in expanded mode even if defaultCount > 0 */
  startExpanded?: boolean;
  /**
   * When true, the center renders the count and label inline as a single line
   * (e.g. "1 in delivery") instead of stacking them. Useful for compact carousel tiles.
   */
  inlineLabel?: boolean;
...
export const QuantityStepper = React.forwardRef<HTMLDivElement, QuantityStepperProps>(
  (
    ...
    const initialMode: StepperMode = startExpanded && defaultCount > 0 ? 'expanded' : defaultCount > 0 ? 'collapsed' : 'initial';
    const [mode, setMode] = useState<StepperMode>(initialMode);
    const [hasInteracted, setHasInteracted] = useState(startExpanded && defaultCount > 0);
    ...
    if (isInitial) {
      ...
      if (inlineLabel) {
        centerContent = (
          <span className={styles.inlineCountLabel}>
            <span className={styles.countValue}>{count}</span>
            {hasInteracted && <span className={styles.labelText}>in {countLabel}</span>}
          </span>
        );
      } else {
        centerContent = (
    ...
    return (
      <div ref={ref} className={[styles.quantityStepper, className].filter(Boolean).join(' ')}>
        <div
